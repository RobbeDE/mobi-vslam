...
class MobiNavigator:
    ...
    def set_goal(self, goal_cell: tuple, grid: OccupancyGrid):
        ...
        if self._nav_thread is None or not self._nav_thread.is_alive():
            ...
            self._nav_thread = threading.Thread(target=self.navigate, args=(grid,), daemon=True)
    ...
    def navigate(self, grid: OccupancyGrid):
        ...
        while not self._stop_event.is_set():
            ...
            angle_error = (angle_to_goal - robot_heading + np.pi) % (2 * np.pi) - np.pi
